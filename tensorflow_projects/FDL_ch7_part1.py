###Fundamentals of Deep learning chapter 7 part 1 
###Models for sequence analysis:
##Analyzing variable-length inputs:

##Tackling seq2seq with neural n-grams:
#Transforming sequences into parts of speech tags (as in the Brown tag 
#set and the pen treebank tag set). 
#These are particularly used for translating text between languages, transcription 
#and text summarization. 

##Implementing a Part of speech tagger: 
#will need to download the gensim package which contains a 
#corpus of pre trained phrases from google news. 
#And in addition the author said that the POS tag set used has 
#a total of 44 tags. 
import gensim 
import leveldb 
import re 
...
#preprocessing for the parts of speech tags and the google news 
#dataset:
print("Loading Pretrained Word2vec Model") 

model = gensim.models.KeyedVectors.load_word2vec_format("GoogleNews-vectors-negative300.bin.gz", binary=True) 

with open("data/pos_data/train.txt") as f:
...
with open("data/pos_data/test.txt") as f:
...
#Caching the vocabulary words in memory so that you don't have to constantly 
#reload the google_news word vector assembly.
counter = 0 
...
for word in dataset_vocab:
	...
	with open("processed.txt") as f:
		...
		train_dataset = [element.split() for element in train_dataset if 
		len(element.split()) > 0] 
